...
type EnhancedBugReport = BugReport & {
  assignee: {
    id: number;
    name: string;
    avatar: string | null;
...
export default function BugReportsTable() {
  ...
  const { data, isLoading, error } = useQuery<EnhancedBugReport[]>({
  ...
  // Get the filtered bugs based on current filters
    ...
    <Card className="overflow-hidden lg:col-span-2">
      <CardHeader className="px-4 py-5 sm:px-6">
        <div className="flex items-center justify-between">
          ...
          <div>
            <Link href="/bug-reports?new=true">
      ...
      <div className="overflow-x-auto">
        <Table>
          ...
          <TableBody>
            ...
            {!isLoading && !error && paginatedBugs.length === 0 && (
              <TableRow>
            ...
            {!isLoading && !error && paginatedBugs.map((bug) => (
              <TableRow 
                key={bug.id} 
                className="hover:bg-gray-50 cursor-pointer"
              >
                <TableCell className="px-6 py-4 whitespace-nowrap">
                  <Link href={`/bug-reports/${bug.id}`}>
                    <div className="text-sm text-gray-900 hover:text-blue-600 cursor-pointer">{bug.bugId}</div>
                  </Link>
                </TableCell>
                <TableCell className="px-6 py-4">
                  <Link href={`/bug-reports/${bug.id}`}>
                    <div className="text-sm text-blue-600 hover:text-blue-800 hover:underline">{bug.title}</div>
                  </Link>
                </TableCell>
                <TableCell className="px-6 py-4 whitespace-nowrap">
                  ...
                  {bug.assignee ? (
                    <div className="flex items-center">
                      <Avatar className="h-8 w-8">
                        <AvatarImage src={bug.assignee.avatar || undefined} alt={bug.assignee.name} />
                        <AvatarFallback>{bug.assignee.name.charAt(0)}</AvatarFallback>
                      </Avatar>
                      <div className="ml-3">
