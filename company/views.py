...
def validate_company_user(func):
    def wrapper(self, request, company_uuid, *args, **kwargs):
        company = (
            Company.objects.filter(Q(admin=request.user) | Q(managers__in=[request.user]))
            .filter(uuid=company_uuid)
            .first()
        )
        ...
        return func(self, request, company.uuid, *args, **kwargs)
...
class CompanyDashboardAnalyticsView(View):
    ...
    def get_general_info(self, company_uuid):
        total_company_bugs = Issue.objects.filter(domain__company__uuid=company_uuid).count()
        total_bug_hunts = Hunt.objects.filter(domain__company__uuid=company_uuid).count()
        total_domains = Domain.objects.filter(company__uuid=company_uuid).count()
        total_money_distributed = Issue.objects.filter(
            domain__company__uuid=company_uuid
    ...
    def get_bug_report_type_piechart_data(self, company_uuid):
        bug_report_type = (
            Issue.objects.values("label")
            .filter(domain__company__uuid=company_uuid)
    ...
    def get_reports_on_domain_piechart_data(self, company_uuid):
        report_piechart = (
    ...
    def get_current_year_monthly_reported_bar_data(self, company_uuid):
        # returns chart data on no of bugs reported monthly on this company for current year

        current_year = timezone.now().year
        data_monthly = (
            Issue.objects.filter(domain__company__uuid=company_uuid, created__year=current_year)
    ...
    def bug_rate_increase_descrease_weekly(self, company_uuid, is_accepted_bugs=False):
    ...
    def get_spent_on_bugtypes(self, company_uuid):
        spent_on_bugtypes = (
            Issue.objects.values("label")
            .filter(domain__company__uuid=company_uuid)
    ...
    def get(self, request, company_uuid, *args, **kwargs):
        companies = (
            Company.objects.values("name", "uuid")
        ...
        context = {
            ...
            "spent_on_bugtypes": self.get_spent_on_bugtypes(company_uuid),
        }
        self.get_spent_on_bugtypes(company_uuid)
...
class AddDomainView(View):
    ...
    @validate_company_user
    def post(self, request, company_uuid, *args, **kwargs):
        domain_data = {
        ...
        if domain_data["name"] is None:
            ...
            return redirect("add_domain", company_uuid)
        ...
        company_obj = Company.objects.get(uuid=company_uuid)
        ...
        return redirect("company_manage_domains", company_uuid)

    @validate_company_user
    def delete(self, request, company_uuid, *args, **kwargs):
...
class CompanyDashboardManageRolesView(View):
    ...
    def post(self, request, company_uuid, *args, **kwargs):
        ...
        if domain is None:
            ...
            return redirect("company_manage_roles", company_uuid)
...
class EndBughuntView(View):
    def get(self, request, pk, *args, **kwargs):
        ...
        hunt.save()
        company = hunt.domain.company.uuid
...
class AddHuntView(View):
    def edit(self, request, company_uuid, companies, domains, hunt_id, *args, **kwargs):
    ...
    def get(self, request, company_uuid, *args, **kwargs):
        ...
        domains = Domain.objects.values("id", "name").filter(company__uuid=company_uuid)

        if hunt_id is not None:
            return self.edit(request, company_uuid, companies, domains, hunt_id, *args, **kwargs)
    ...
    def post(self, request, company_uuid, *args, **kwargs):
        ...
        if domain is None:
            ...
            return redirect("add_bughunt", company_uuid)
        ...
        messages.success(request, "successfully added the managers")
        return redirect("company_manage_bughunts", company_uuid)
