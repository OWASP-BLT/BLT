...
def is_api_key_valid(api_key):
    ...
    try:
        openai.Completion.create(prompt="Hello", model="gpt-3.5-turbo-instruct", max_tokens=1)
        return True
...
def load_document(file_path):
    ...
    return Loader(str(file_path)).load()  # Convert Path to string


def load_directory(dir_path):
    dir_path = str(Path(dir_path))  # Convert Path to string
    return DirectoryLoader(dir_path).load()


def embed_documents_and_save(embed_docs):
    db_folder_path = "faiss_index"  # Use a relative path
    ...
    if default_storage.exists(db_folder_path) and default_storage.listdir(db_folder_path):
        ...
        db_files = [
            default_storage.path(f"{db_folder_path}/{file_name}")
