...
from website.models import Organization, Project, Repo, Tag, Contributor
    ...
    def handle(self, *args, **options):
        ...
        for row_index, row in enumerate(rows, start=1):
            ...
            if not self.validate_github_repo(repo_url, headers, delay_on_rate_limit, max_rate_limit_retries):
                self.stderr.write(self.style.WARNING(f"Invalid or inaccessible Repo URL: {repo_url}. Skipping row {row_index}."))
                continue

            # Fetch complete GitHub repo data
            repo_info = self.fetch_github_repo_data(repo_url, headers, delay_on_rate_limit, max_rate_limit_retries, is_wiki=True, is_main=False)
            if not repo_info:
                self.stderr.write(self.style.WARNING(f"Failed to fetch complete data for {repo_url}. Skipping row {row_index}."))
                continue
            ...
            with transaction.atomic():
                ...
                try:
                    ...
                    self.stdout.write(
                        self.style.WARNING(
                            f"Failed to create project due to duplicate data. Skipping..."
                        )
                    )
                    continue
                ...
                try:
                    ...
                    try:
                        repo = Repo.objects.create(
                            ...
                            last_updated=parse_datetime(repo_info.get('last_updated')) if repo_info.get('last_updated') else None,
                            watchers=repo_info.get('watchers', 0),
                            primary_language=repo_info.get('primary_language', ''),
                            license=repo_info.get('license', ''),
                            last_commit_date=parse_datetime(repo_info.get('last_commit_date')) if repo_info.get('last_commit_date') else None,
                            ...
                            release_datetime=parse_datetime(repo_info.get('release_datetime')) if repo_info.get('release_datetime') else None,
                        )
                    except IntegrityError:
                        self.stdout.write(
                            self.style.WARNING(
                                f"Failed to create wiki repo due to duplicate data. Skipping..."
                            )
                        )
                        continue
                ...
                for idx, code_url in enumerate(code_urls, start=1):
                    ...
                    if not self.validate_github_repo(code_url, headers, delay_on_rate_limit, max_rate_limit_retries):
                        self.stderr.write(self.style.WARNING(f"Invalid or inaccessible Code Repo URL: {code_url}. Skipping."))
                        continue

                    # Fetch the fetch_github_repo_data for the code repo and for the first one set is_main=True
                    code_repo_info = self.fetch_github_repo_data(code_url, headers, delay_on_rate_limit, max_rate_limit_retries, is_wiki=False, is_main=idx == 1)

                    if not code_repo_info:
                        self.stderr.write(self.style.WARNING(f"Failed to fetch complete data for {code_url}. Skipping."))
                        continue
                    ...
                    try:
                        ...
                        try:
                            ...
                            code_repo = Repo.objects.create(
                                ...
                                last_updated=parse_datetime(code_repo_info.get('last_updated')) if code_repo_info.get('last_updated') else None,
                                watchers=code_repo_info['watchers'],
                                primary_language=code_repo_info['primary_language'],
                                license=code_repo_info['license'],
                                last_commit_date=parse_datetime(code_repo_info.get('last_commit_date')) if code_repo_info.get('last_commit_date') else None,
                                ...
                                release_datetime=parse_datetime(code_repo_info.get('release_datetime')) if code_repo_info.get('release_datetime') else None,
                            )
                        except IntegrityError:
                            self.stdout.write(
                                self.style.WARNING(
                                    f"Failed to create code repo due to duplicate data. Skipping..."
                                )
                            )
                            continue
                    ...
                    if not code_repo:
                        self.stderr.write(self.style.WARNING(f"Failed to create/update Code Repo for {code_url}. Skipping."))
                        continue
                    ...
                    self.stdout.write(
                        self.style.SUCCESS(f"   -> Added/Updated Repo: {code_repo.name} ({code_url})")
                    )
        ...
        for attempt in range(max_retries):
            try:
                ...
                if response.status_code == 200:
                    ...
                    self.stderr.write(self.style.WARNING(f"Unexpected status code {response.status_code} for URL: {repo_url}"))
                    return False
            except requests.exceptions.RequestException as e:
                self.stderr.write(self.style.WARNING(f"Network error while validating {repo_url}: {e}. Retrying in {delay} seconds..."))
                time.sleep(delay)
                continue
        return False
