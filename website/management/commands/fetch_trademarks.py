...
class Command(LoggedBaseCommand):
    help = "Fetch trademark information for a random organization and store it in the database"
    ...
    def handle(self, *args, **kwargs):
        ...
        while retries > 0:
            try:
                ...
                retries -= 1
                if retries == 0:
                    self.stdout.write(self.style.ERROR(f"Failed to fetch data for {name} after 3 retries: {str(e)}"))
                else:
                    self.stdout.write(self.style.WARNING(f"Retrying for {name}... ({3-retries} attempts remaining)"))
