...
def mask_email(email):
    """
    Return a masked version of an email address to limit exposure of personally identifiable information.
    
    Parameters:
        email (str): The email to mask. If `email` is not a string or does not contain `"@"`, the function returns `"***"`.
    
    Returns:
        str: `"***"` for invalid input; otherwise a masked email of the form `<first-char>***@domain` (or `"***@domain"` when the local part has length 1 or less).
    """
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        """
        Run the deduplication process for allauth EmailAddress records to consolidate case-insensitive duplicate emails.
        
        Finds emails that appear more than once (case-insensitive) and for each group keeps one EmailAddress while merging verified and primary flags, ensuring the kept record is associated with a user, preventing cross-user merges, and avoiding removal that would leave a user without any email addresses. Skips and reports groups with irreconcilable data issues and performs all changes inside a database transaction.
        """
