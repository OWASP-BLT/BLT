...
from website.github_verification import award_github_linking_tokens, extract_github_username, verify_github_linkback
    ...
    def handle(self, *args, **options):
        ...
        for profile in profiles:
            ...
            if verification_result["verified"]:
                verified_count += 1
                self.stdout.write(self.style.SUCCESS(f"✓ Verified linkback in {verification_result['found_in']}"))

                if not dry_run:
                    success = award_github_linking_tokens(profile.user, github_username)
                    if success:
                        awarded_count += 1
