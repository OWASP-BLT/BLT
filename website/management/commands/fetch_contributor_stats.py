...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        user_stats = defaultdict(lambda: {
            "commits": 0, "issues_opened": 0, "issues_closed": 0,
            "prs": 0, "comments": 0, "assigned_issues": 0,
        })

        # Fetch and process data
        self.fetch_and_update_data("pulls", user_stats, headers, owner, repo, since, start_date)
        self.fetch_and_update_data("issuesopen", user_stats, headers, owner, repo, since, start_date)
        self.fetch_and_update_data("issuesclosed", user_stats, headers, owner, repo, since, start_date)
