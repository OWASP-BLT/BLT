...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if not github_token and (options["stop_closed"]):
            self.stdout.write(self.style.ERROR("GitHub token required for --stop-closed option"))
            return
    ...
    def stop_stale_timers(self, timers, dry_run):
        ...
        for timer in stale_timers:
            ...
            self.stdout.write(
                f"  Timer {timer.id} for {timer.user.username}: " f"{duration.total_seconds() / 3600:.1f} hours"
            )
    ...
    def stop_closed_issue_timers(self, timers, github_token, dry_run):
        ...
        timers_with_issues = timers.exclude(github_issue_number__isnull=True).exclude(github_repo__isnull=True)

        self.stdout.write(f"Checking {timers_with_issues.count()} timers with GitHub issues")
        ...
        for timer in timers_with_issues:
            try:
                ...
                if response.status_code == 200:
                    ...
                    self.stdout.write(self.style.WARNING(f"  Timer {timer.id}: Issue not found (may be deleted)"))
                else:
                    self.stdout.write(
                        self.style.WARNING(f"  Timer {timer.id}: GitHub API error {response.status_code}")
                    )

            except requests.RequestException as e:
                self.stdout.write(self.style.ERROR(f"  Timer {timer.id}: Request failed - {str(e)}"))
            except Exception as e:
                self.stdout.write(self.style.ERROR(f"  Timer {timer.id}: Error - {str(e)}"))
