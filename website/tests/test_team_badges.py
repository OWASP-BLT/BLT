...
class TeamBadgeModelTests(TestCase):
    ...
    def test_team_badge_creation(self):
        team_badge = TeamBadge.objects.create(team=self.team, badge=self.badge)
...
class TeamBadgeAssignmentTests(TestCase):
    ...
    def setUp(self):
        ...
        self.badge2 = Badge.objects.create(
            title="Team Leader", description="Team leadership badge", type="manual", scope="topuser_team"
        )
    ...
    def test_assign_multiple_badges_to_team(self):
        ...
        self.assertEqual(TeamBadge.objects.filter(team=self.team).count(), 2)
