...
class SpamDetectionUnitTests(TestCase):
    ...
    def test_spam_detection_disabled_without_api_key(self):
        ...
        result = detector.check_bug_report(
            title="Test Bug", description="This is a test description.", url="https://example.com"
        )
        ...
        self.assertEqual(
            ...
            "Reason should indicate that spam detection is not available.",
        )
    ...
    def test_check_bug_report_legitimate_report(self, mock_genai_client):
        ...
        self.assertIn(
            "technical details", result["reason"], "Reason should reflect why the report is considered legitimate."
        )
    ...
    def test_check_bug_report_obvious_spam(self, mock_genai_client):
        ...
        self.assertIn(
            "promotional language", result["reason"], "Reason should reflect why the report is considered spam."
        )
    ...
    def test_check_bug_report_ambiguous_report(self, mock_genai_client):
        ...
        detector = SpamDetection()
        result = detector.check_bug_report(
            title="It's broken", description="The website is not working.", url="https://example.com"
        )
    ...
    def test_check_bug_report_api_error_handling(self, mock_genai_client):
        ...
        self.assertEqual(
            result["reason"], "Error parsing spam detection response", "Reason should indicate that an error occurred."
        )
    ...
    def test_check_bug_report_invalid_json_response(self, mock_genai_client):
        ...
        self.assertEqual(
            result["reason"], "Invalid spam detection response", "Reason should indicate an invalid response."
        )
    ...
    def test_get_system_prompt_contains_all_details(self):
        ...
        detector = SpamDetection()
        prompt = detector._get_system_prompt(title="Test Title", desc="Test Description", url="https://example.com")
...
class IssueCreateSpamIntegrationTests(TestCase):
    ...
    @unittest.skip(
        "Integration test has issues with Django test client file upload - spam detection is covered by unit tests"
    )
    @patch("website.views.issue.SpamDetection")
    ...
    def test_issue_with_high_spam_score_is_hidden(self, mock_captcha_form, mock_spam_detection):
        ...
        self.assertEqual(
            issue.spam_reason, "This report contains promotional content.", "The spam reason should be saved."
        )
...
class IssueAdminSpamActionTests(TestCase):
    ...
    def test_unmark_as_spam_admin_action(self):
        ...
        self.assertFalse(
            self.spam_issue.is_hidden, "The issue should no longer be hidden after being unmarked as spam."
        )
        self.assertTrue(
            self.spam_issue.verified, "The issue should be marked as verified after being unmarked as spam."
        )
