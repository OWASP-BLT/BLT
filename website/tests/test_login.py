...
def test_login_with_invalid_email_shows_error(self):
    ...
    response = self.client.post("/accounts/login/", {"login": "nonexistent@example.com", "password": "wrongpassword"})
    self.assertContains(response, "non_field_errors")
...
def test_login_with_email_success(self):
    ...
    response = self.client.post("/accounts/login/", {"login": "test@example.com", "password": "password123"})
