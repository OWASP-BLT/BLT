...
(function() {
    ...
    function createPrizeElement(prize_data) {
        const prize_name_sanitized = prize_data.prize_name.trim().substring(0, 8) + '...';
        let prize_description_sanitized = prize_data.prize_description.trim().substring(0, 55) + '...';
        ...
        removeBtn.classList.add("absolute", "top-2", "right-2", "text-red-500", "hover:text-red-700", "font-bold", "text-xl");
        removeBtn.textContent = "×";
        removeBtn.onclick = function() { remove_prize(prize_data.id); };
        ...
        const content = `
            ...
            <div class="mb-4">
                ...
                <p class="text-gray-800">$${escapeHTML(prize_data.cash_value)}</p>
            </div>
            <div class="mb-4">
                <p class="text-gray-800 font-bold">Number of Winning Projects</p>
                <p class="text-gray-600">${escapeHTML(String(sanitizedNumberOfWinningProjects))}</p>
            </div>
            <div class="mb-4">
                <p class="text-gray-800 font-bold">Reward All Valid Submission</p>
                <p class="text-gray-600">${prize_data.every_valid_submissions ? 'Yes' : 'No'}</p>
            </div>
            <div class="mb-4">
        ...
        prizeContainer.innerHTML += content;
    ...
    function updatePrize(prizeId, organizationId) {
        ...
        // Show loading state
        const updateButton = document.getElementById('add_prize_button');
        const originalText = updateButton.innerText;
        updateButton.disabled = true;
        updateButton.innerText = 'Updating...';
        
        // Make AJAX call to update the prize
        ...
        .then(data => {
            if (data.success) {
                ...
                alert(data.message || "Failed to update prize. Please try again.");
            }
        })
        .catch(error => {
            console.error('Error:', error);
            alert("Network error occurred while updating the prize. Please check your connection and try again.");
        })
        .finally(() => {
