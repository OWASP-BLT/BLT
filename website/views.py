...
def filter_contributions_by_year(request, year):
    contributions = Contribution.objects.filter(created__year=year)
    return render(request, "filtered_contributions.html", {"contributions": contributions})
...
def contributors_view(request, *args, **kwargs):
    ...
    if selected_year:
        contributions = Contribution.objects.filter(created__year=selected_year)
    else:
