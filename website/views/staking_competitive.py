...
def calculate_individual_user_progress(user, challenge, pool_entry=None):
    ...
    challenge_mapping = {
        "find_bugs": (Issue, lambda u, from_date: Issue.objects.filter(
            user=u, 
            created__gte=from_date if from_date else timezone.now() - timedelta(days=365*10)
        ).count()),
        "report_bugs": (IpReport, lambda u, from_date: IpReport.objects.filter(
            user=u,
            created__gte=from_date if from_date else timezone.now() - timedelta(days=365*10)  
        ).count()),
        "log_time": (TimeLog, lambda u, from_date: TimeLog.objects.filter(
            user=u,
            created__gte=from_date if from_date else timezone.now() - timedelta(days=365*10)
        ).count()),
    }
...
def staking_home(request):
    ...
    user_total_staked = StakingEntry.objects.filter(
        user=request.user, status='active'
    ).aggregate(total=Sum('staked_amount'))['total'] or 0
    ...
    user_total_winnings = StakingEntry.objects.filter(
        user=request.user, status='won'
    ).aggregate(total=Sum('actual_reward'))['total'] or 0
    ...
    try:
        with transaction.atomic():
            ...
            if success:
                ...
                if pool.is_full:
                    messages.info(request, f"Pool is now full! Challenge will begin soon.")
        ...
        with transaction.atomic():
            for entry in active_entries:
                ...
                if success and entry.status == 'won':
                    messages.success(request, f"🎉 Congratulations! You won {entry.pool.name} and earned {entry.actual_reward} BACON!")
                elif success:
                    messages.info(request, f"Challenge completed for {entry.pool.name}, but someone else finished first.")
...
def my_staking(request):
    ...
    active_entries = StakingEntry.objects.filter(
        user=request.user,
        status='active'
    ).select_related('pool', 'pool__challenge').order_by('-created_at')
    
    # Get user's completed entries
    completed_entries = StakingEntry.objects.filter(
        user=request.user,
        status__in=['won', 'lost']
    ).select_related('pool', 'pool__challenge').order_by('-created_at')
    
    # Get user's transaction history
    transactions = StakingTransaction.objects.filter(
        user=request.user
    ...
    total_winnings = completed_entries.filter(status='won').aggregate(
...
def staking_leaderboard(request):
    ...
    # Get recent winners
    recent_winners = StakingEntry.objects.filter(
        status='won'
    ).select_related('user', 'pool').order_by('-completion_time')[:10]
    ...
    total_prize_pool = StakingPool.objects.filter(status__in=['open', 'full', 'active']).aggregate(
        total=Sum('stake_amount') * 2  # Assuming head-to-head pools
    )['total'] or 0
    total_distributed = StakingEntry.objects.filter(status='won').aggregate(
        total=Sum('actual_reward')
    )['total'] or 0
    ...
    if request.method == 'POST':
        try:
            ...
            with transaction.atomic():
                ...
                messages.success(request, f"Pool '{name}' created successfully! You've automatically joined with {stake_amount} BACON.")
