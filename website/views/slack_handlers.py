        ...
        if command == "/discover":
            ...
            if search_term.startswith("user "):
                ...
                response = JsonResponse({
                    "response_type": "ephemeral",
                    "text": "I've sent you the user profile in a DM! 👤"
                })
                ...
                response = JsonResponse({
                    "response_type": "ephemeral",
                    "text": "🌍 I'll send you the chapter information in a DM shortly!"
                })
...
def get_gsoc_overview(workspace_client, user_id, search_term, activity, team_id):
    try:
        ...
        activity.error_message = str(e)
        activity.save()
        return JsonResponse({"response_type": "ephemeral", "text": f"❌ Error: Check the logs for more details."}, status=400)
...
def get_user_profile(username, workspace_client, user_id):
    ...
    try:
        ...
        if not GITHUB_TOKEN:
            return [{
                "type": "section",
                "text": {"type": "mrkdwn", "text": "⚠️ GitHub API token not configured. Please contact the administrator."}
            }]
        ...
        if gh_response.status_code == 404:
            return [{
                "type": "section",
                "text": {"type": "mrkdwn", "text": f"❌ GitHub user '{username}' not found."}
            }]
        elif gh_response.status_code != 200:
            return [{
                "type": "section",
                "text": {"type": "mrkdwn", "text": "❌ Failed to fetch GitHub profile."}
            }]
        ...
        blocks = [
            {
                "type": "header",
                "text": {"type": "plain_text", "text": f"🔍 OWASP Profile: {profile.get('name', username)}", "emoji": True}
            },
            {"type": "divider"},
            {
                "type": "section",
                ...
                } if profile.get('avatar_url') else None
            }
        ]
        ...
        if contributions:
            blocks.extend([
                ...
                {
                    "type": "section",
                    "text": {"type": "mrkdwn", "text": "*🛠 OWASP Contributions*"}
                },
                {
                    "type": "section",
                    "text": {"type": "mrkdwn", "text": contributions}
                }
            ])
        ...
        if gsoc_info:
            blocks.extend([
                ...
                {
                    "type": "section",
                    "text": {"type": "mrkdwn", "text": "*🎓 GSoC Involvement*"}
                },
                {
                    "type": "section",
                    "text": {"type": "mrkdwn", "text": gsoc_info}
                }
            ])
        ...
        return [{
            "type": "section",
            "text": {"type": "mrkdwn", "text": "❌ An error occurred while fetching user profile."}
        }]
...
def get_gsoc_involvement(username):
    ...
    try:
        ...
        mentor_projects = [
            project for project in GSOC_PROJECTS 
            if username.lower() in project['mentor'].lower()
        ]
        
        if mentor_projects:
            projects_text = "\n".join(
                f"• Mentor for *{project['title']}*" 
...
def get_chapter_overview(workspace_client, user_id, search_term, activity):
    ...
    try:
        headers = get_github_headers()
        if not GITHUB_TOKEN:
            send_dm(workspace_client, user_id, "Error", [{
                "type": "section",
                "text": {"type": "mrkdwn", "text": "⚠️ GitHub API token not configured. Please contact the administrator."}
            }])
            return
        ...
        params = {
            "q": "org:OWASP www-chapter in:name",
            "sort": "updated",
            "order": "desc",
            "per_page": 100
        }
        ...
        if response.status_code != 200:
            send_dm(workspace_client, user_id, "Error", [{
                "type": "section",
                "text": {"type": "mrkdwn", "text": "❌ Failed to fetch chapter repositories."}
            }])
            return
...
def send_chapter_page(client, user_id, chapters):
    ...
    try:
        blocks = [
            {
                "type": "header",
                "text": {"type": "plain_text", "text": "🌍 OWASP Chapters", "emoji": True}
            },
            {"type": "divider"}
        ]
...
def get_chapter_details(repo_name, headers, workspace_client, user_id):
    ...
    try:
        ...
        if repo_response.status_code == 404:
            return JsonResponse({
                "response_type": "ephemeral",
                "text": f"❌ Chapter repository '{repo_name}' not found."
            })
        elif repo_response.status_code != 200:
            return JsonResponse({
                "response_type": "ephemeral",
                "text": "❌ Failed to fetch chapter details."
            })
        ...
        return JsonResponse({
            "response_type": "ephemeral",
            "text": "❌ An error occurred while fetching chapter details."
        })
...
def handle_chapter_pagination(action, body, client):
    ...
    try:
        ...
        return JsonResponse({
            "response_type": "ephemeral",
            "text": "❌ An error occurred while navigating chapters."
        })
