...
def handle_discover_command(ack, client, command):
    try:
        ...
        if search_term:
            ...
            if not matched:
                send_dm(
                    client, command["user_id"], f"No OWASP projects found matching '{search_term}'."
                )
                return
            ...
            try:
                ...
                if gh_response.status_code == 200:
                    ...
                    send_dm(
                        client, command["user_id"], "Failed to fetch repositories from OWASP-BLT."
                    )

            except Exception as e:
                logger.error(f"Error processing repositories: {e}")
                send_dm(
                    client, command["user_id"], "An error occurred while processing your request."
                )
...
def send_paged_results(client, user_id, search_term):
    ...
    options = [
        {"text": {"type": "plain_text", "text": project["name"]}, "value": project["owner_repo"]}
