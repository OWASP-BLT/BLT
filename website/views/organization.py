...
def admin_organization_dashboard(request, template="admin_dashboard_organization.html"):
    ...
    if user.is_superuser:
        ...
        organizations_qs = (
            Organization.objects.filter(Q(admin=user) | Q(managers=user))
            .distinct()
            .select_related("admin", "subscription")
        )
    ...
    if not selected_organization:
        ...
        if selected_organization_id:
            ...
            selected_organization = next(
                (org for org in organizations_list if org.pk == selected_organization_id), None
            )
            if not selected_organization:
