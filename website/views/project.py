...
class ProjectBadgeView(APIView):
    ...
    def get(self, request, slug):
        ...
        visit_counts_query = (
            IP.objects.filter(path=request.path, created__date__gte=start_date)
        ...
        visits_map = {entry["date"]: entry["visit_count"] for entry in visit_counts_query}

        # Create a full list of dates and counts for the last 30 days
        dates = [start_date + timedelta(days=i) for i in range(DAY_RANGE)]
        counts = [visits_map.get(date, 0) for date in dates]
        ...
        # Ensure we always have space for 30 bars, and ensure max_count is at least 1
        bar_width = chart_width / (DAY_RANGE * 2)  # Space for 30 days with gaps
