...
if __name__ == "__main__":
    payload_ok = {
        ...
        "number": 1,
        "pull_request": {
            "url": "http://api.github.com/repos/test/repo/pulls/1",
            "diff_url": "http://api.github.com/repos/test/repo/pulls/1.diff",
            "id": 123,
            "state": "open",
            "user": {"login": "testuser"},
            "title": "Test PR",
            "body": "Testing branch verification.",
            "draft": False,
            "head": {"ref": "feature-branch"},
            "base": {"ref": "main", "repo": {"full_name": "test/repo", "default_branch": "main"}},
        },
    }
    ...
    payload_bad = {
        **payload_ok,
        "pull_request": {
            **payload_ok["pull_request"],
            "base": {**payload_ok["pull_request"]["base"], "ref": "develop"},
        },
    }

    print("Testing with matching branches (should succeed):")
    try:
        pr = PullRequest(payload_ok)
        print("Success: No branch mismatch detected.")
    except BranchMismatchError as e:
        print("BranchMismatchError:", e)

    print("\nTesting with mismatched branches (should fail):")
    try:
        pr = PullRequest(payload_bad)
        print("Error: BranchMismatchError was not raised as expected.")
    except BranchMismatchError as e:
        print("BranchMismatchError:", e)
