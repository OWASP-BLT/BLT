...
def process_similarity_analysis(repo1_path, repo2_path,  use_codebert=False):
    ...
    for model1 in models1:
        for model2 in models2:
            if use_codebert:
                model_similarity = analyze_code_similarity_with_codebert(model1["name"], model2["name"])
            else:
...
def analyze_code_similarity_with_codebert(code1, code2):
    ...
    """
    inputs = tokenizer(f"{code1} [SEP] {code2}", return_tensors="pt", truncation=True, max_length=512)
                ...
                with open(file_path, 'r') as f:
                    ...
                    for line in lines:
                        ...
                        if inside_model:
                            ...
                            match_complex = re.match(r'^\s*(\w+)\s*=\s*models\.(ForeignKey|ManyToManyField|OneToOneField)\((.*)\)', line)
                            if match_complex:
                                field_name = match_complex.group(1)
                                field_type = match_complex.group(2)
                                field_params = match_complex.group(3).strip()
                                fields.append({'field_name': field_name, 'field_type': field_type, 'parameters': field_params})
...
def compare_model_fields(model1, model2):
    ...
    return {
        ...
        'field_similarity': round(field_similarity, 2)
    }