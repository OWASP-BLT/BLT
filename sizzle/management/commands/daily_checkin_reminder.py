...
from sizzle.utils.model_loader import get_notification_model, get_userprofile_model
...
class Command(SizzleBaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if UserProfile is None:
            self.log_error("UserProfile model not configured or available. " "Check SIZZLE_USERPROFILE_MODEL setting.")
            return

        Notification = get_notification_model()
        if Notification is None:
            self.log_error(
                "Notification model not configured or available. " "Check SIZZLE_NOTIFICATION_MODEL setting."
            )
            return
